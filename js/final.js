...
const DEBUTS = ["t'as l'aura de","t'es officiellement","sur ma vie t'es","scientifiquement prouvé : t'es",
  "franchement t'es","l'État confirme que t'es","d'après mes sources t'es"];
const MILIEUX = [
  "6-7 fois plus stylée que la moyenne nationale","plus rare qu'un TGV à l'heure",
  "la seule personne à qui le mot « rizz » rend justice","plus légendaire qu'un Tasty Crousty",
  "meilleure que le wifi gratuit dans un TER","plus solide qu'un Nokia 3310 tombé du 6e étage",
  "l'équivalent humain d'un vendredi 17h","trop drôle pour être légale dans 6-7 départements",
  "capable de rendre un lundi matin supportable","la définition du mot « validée » dans le Larousse 2027",
  "plus précieuse qu'un chargeur retrouvé chez quelqu'un d'autre","au-dessus du game depuis la maternelle",
  "la raison pour laquelle le groupe tient encore debout","plus attendue que le générique de Fort Boyard",
  "la seule à pouvoir dire « wesh alors » avec élégance","plus rare qu'un Tasty Crousty en 2026",
  "plus généreuse que Nasdas un soir de story","plus culte que Swan et Néo pour une génération entière",
...
function remplirBilan(){
  ...
  const lignes = [
    ...
    ["Manche de mémoire atteinte",  ETAT.mancheSimonMax + " / 5"],
  ...
  else if(ETAT.mancheSimonMax >= 5 && !ETAT.indiceMdp)
...
document.getElementById('b13').onclick = function(){
  n67 = 0; ritOK = false; ritGele = false; fausseRAZ = false;
  qi = 0; essaisNom = 0;
  ...
  clearInterval(tMelange67); resetCadeau(); rangerPads(); retrecirBouton();
